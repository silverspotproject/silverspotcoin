...
// Copyright (c) 2013-2014 The Silverspotcoin developers
...
uint256 hashGenesisBlock("0x836040162b3ecf2349d7cd22169984f64a43dac4aff3d695bc086529fece0640");
...
const string strMessageMagic = "Silverspotcoin Signed Message:\n";
...
{
    // Silverspotcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Silverspotcoin: 1 days
static const int64 nTargetSpacing = 1 * 60; // Silverspotcoin: 1 Minute Blocks
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Silverspotcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (fTestNet) {
                ...
                if (pindexLast->nHeight+1 >= 32000) { DiffMode = 2; }
        }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Silverspotcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Silverspotcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Sunny day in London, Thailand Coup";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1400779939;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087347361;
        ...
        assert(block.hashMerkleRoot == uint256("0xe67771015b637cb37e1dc61dd5f54eae52318aac184efc3458f228346692dfb7"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Silverspotcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SilverspotcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SilverspotcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SilverspotcoinMiner(CWallet *pwallet)
{
    printf("SilverspotcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("silverspotcoin-miner");
    ...
    try { loop {
        ...
        printf("Running SilverspotcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SilverspotcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SilverspotcoinMiner, pwallet));
}
